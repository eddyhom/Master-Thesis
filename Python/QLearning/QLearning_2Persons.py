...
class GridWorld(object):
    ...
    def setReward(self, newState, newQuadrant, resultingPosition):
        ...
        '''if (not self.state[2] and newState[2]) or (self.state[2] and newState[2]):  # If we in green zone and entered red zone
            if newState[3] == newQuadrant:  # If close and going towards it, big punishment
                return -30 * beenThere
            else:  # red zone and not going towards negative
                if self.getDist(resultingPosition, self.negative) <= self.getDist(self.agentPosition, self.negative):
                    return -7 * beenThere  # Wrong direction but getting closer
                else:  # Wrong direction and getting farther away
                    return -15 * beenThere
        elif (self.state[2] and not newState[2]) or (not self.state[2] and not newState[2]):  # If we exit red zone
            if newState[1] == newQuadrant:  # If we are going in the right direction - give small punishment
                return -1 * beenThere  # Best Case Scenario where it goes a straight direction
    ...
    def render(self, win):  # Draws the robot and goal in a map
        ...
        pygame.draw.rect(win, (0, 0, 255), (
            50 + self.agentPosition[0] * 40, 50 + self.agentPosition[1] * 40, robot_size,
            robot_size))  # draws robot in blue
        pygame.draw.rect(win, (0, 255, 0), (
            50 + self.goal[0] * 40, 50 + self.goal[1] * 40, robot_size, robot_size))  # draws goal in green

        # If you wanna draw negative person uncomment line below and adapt the names to your own - draw negative person in red
        pygame.draw.rect(win, (255, 0, 0),
                         (50 + self.negative[0] * 40, 50 + self.negative[1] * 40, robot_size, robot_size))  # draws goal
...
if __name__ == '__main__':
    ...
    for i in range(numGames):
        ...
        if i % 5000 == 0:  # and i > stopLearning-1:  # Rend every 2500 iterations
